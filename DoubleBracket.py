...
class DoubleBracket(Bracket):
    ...
    def __init__(self, competitors):
        ...
        self.num_source_nodes = self.num_competitors * 2
    ...
    def create_bracket(self):
        ...
        self.nodes[len(self.nodes) - 5].set_next(self.nodes[len(self.nodes) - 3])
        self.nodes[len(self.nodes) - 4].set_next(self.nodes[len(self.nodes) - 2])
        self.nodes[len(self.nodes) - 3].set_next(self.nodes[len(self.nodes) - 1])
        self.nodes[len(self.nodes) - 2].set_next(self.nodes[len(self.nodes) - 1])
    ...
    def fill_bracket(self):
        ...
        for i in range(math.floor(self.num_competitors - 1)):
            if i < num_comp_nodes / 2:
    ...
    def match_winner(self, node_index):
        ...
        if no_pair == False:  # As long as there is competitor node ready
            ...
            for i in range(self.num_nodes - 3, int((self.num_nodes - 1) / 2), -1):
            ...
            if node_pair in self.winners_bracket_indexes and node_pair != self.num_nodes - 5:
                if node_index % 2 == 0:
                    loser_index = 0
                    count = 0
                    while loser_index == 0:
    ...
    def match_undo(self, node_index):
        ...
        while next_node != None and next_node.get_value() != None:
            if self.nodes.index(next_node) in self.losers:
                ...
                next_node.set_value(None)
            if math.floor(self.nodes.index(next_node) / 2) + int((self.num_nodes - 1) / 2) + 1 < self.num_nodes - 2:
                next_node = self.nodes[math.floor(self.nodes.index(next_node) / 2) + int((self.num_nodes - 1) / 2)]
    ...
    def get_num_levels(self):
        return (math.ceil(math.log(self.num_competitors * 2, 2)) * 2) + 2  # Returns the number of levels the bracket has

    def get_levels(self):
        levels = []
        level = []
        current_node = 0
        node_range = math.floor((self.num_nodes - 2) / 2)
        level_count = math.ceil((self.num_nodes - 2) / 2)
        for i in range(self.num_nodes - 3):  # Separate the nodes into a series of levels as is the structure of the bracket
            node = self.nodes[i]
        ...
        levels.append(level)
        level = []
        level.append(self.nodes[self.num_nodes - 3])
        level.append(self.nodes[self.num_nodes - 2])
        levels.append(level)
        level = []
        level.append(self.nodes[self.num_nodes - 1])
        levels.append(level)
        return levels
