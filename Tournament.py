            ...
            for level in bracket.get_level_list():  # Create bracket in tkinter window using buttons and labels
                ...
                for entry in level:
                    if entry.get_value() != -1:
                        self.buttons.append(tk.Button(self.entries_frame, background="springgreen3", activebackground="springgreen4", fg="black", text=entry.get_value(), font=('Sans-Serif 8 bold'),
                    ...
                    if level[j].get_value() != -1:
                        if j < int(len(level) / 4):
                            self.buttons.append(tk.Button(self.entries_frame, background="springgreen3",
                                                          activebackground="springgreen4", fg="black",
                            ...
                            self.buttons.append(
                                ...
                                tk.Button(self.entries_frame, background="springgreen3", activebackground="springgreen4", fg="black", text=level[j].get_value(), font=('Serif-Sans 8 bold'),
                                          command=lambda node_counter1=node_counter: self.match_result(
                                              node_counter1)))
